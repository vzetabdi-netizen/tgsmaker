...
class EnhancedSVGToTGSBot:
    def __init__(self):
        ...
        # Timestamp of the last "daily limit reached" notice per user.
        # Used to coalesce notices when a user uploads many files at once.
        self.user_limit_notice_at: dict[int, float] = {}
    ...
    async def _handle_rpremium(self, chat_id: int, admin_id: int, parts: list):
        ...
        try:
            ...
            self.db.set_user_plan(uid, 'free', expires_at=None,
                                  granted_by=admin_id, grant_source='admin_remove')
    ...
    async def _notify_daily_limit_reached(self, chat_id: int, user_id: int,
                                          text: str, window: float = 10.0) -> None:
        """
        Send a 'daily limit reached' message at most once per `window` seconds
        per user. This prevents flooding the chat when a user uploads many
        files in a single burst after hitting the limit.
        """
        import time
        now  = time.monotonic()
        last = self.user_limit_notice_at.get(user_id, 0.0)
        if now - last < window:
            return
        self.user_limit_notice_at[user_id] = now
        await self.send_message(chat_id, text)
    ...
    async def _queue_file(self, msg: dict, file_type: str):
        ...
        if not allowed:
            ...
            await self._notify_daily_limit_reached(
                chat_id, user_id,
    ...
    async def _handle_premiumall(self, chat_id: int, admin_id: int, parts: list):
        ...
        try:
            ...
            updated, skipped, updated_uids = self.db.set_plan_all_users(
                plan_id, expires_at, granted_by=admin_id
            )
            plan    = get_plan(plan_id)
            ...
            # Only notify users whose plan was actually changed.
            notified = 0
            ...
            logger.info(
                ...
                f"(skipped {skipped} active-Pro), notified {notified}"
            )
        except ValueError:
    ...
    async def _handle_rpremiumall(self, chat_id: int, admin_id: int, parts: list):
        ...
        try:
            ...
            user_msg = (
                "ℹ️ <b>Plan Update!</b>\n\n"
                "🆓 Your bonus Pro plan from an admin has ended. "
                "Your plan has been changed to <b>Free</b>.\n\n"
